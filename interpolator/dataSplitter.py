...
from interpolator.dataLoader import toTorch, NLLDataset
...
class GridSplitter(Splitter):
    ...
    def _get_unique(self, 
                    arr: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
        Gets the unique elements along an axis

        Args:
            arr (npt.NDArray[np.float64]): The input array

        Returns:
            npt.NDArray[np.float64]: The unique elements
        """
        return np.unique(arr)

    def _get_axis_size(self, arr: npt.NDArray[np.float64]) -> int:
        """
        Gets the number of unique elements along an axis

        Args:
            arr (npt.NDArray[np.float64]): The array to get the size of
