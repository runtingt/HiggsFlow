...
def read(scan, param, files, chop, remove_near_min, rezero,
    ...
    if improve:
        ...
        func = ROOT.TF1('splinefn' + str(NAMECOUNTER), func_method,
...
parser.add_argument('--drop-func', action='store_true')
...
parser.add_argument('--title-right', default='138 fb^{-1} (13 TeV)')
...
parser.add_argument('--suppl',help='mark them as supplementary', action='store_true')
...
if args.others is not None:
    for i, oargs in enumerate(args.others):
        ...
        if args.premade:
            ...
            tmp_gr = tmp_file.Get('main').Clone()
            tmp_file.Close()
        other_scan = BuildScan(args.output,
                      other_POI,
                      [splitargs[0]],
                      int(splitargs[2]),
                      yvals,
...
if args.envelope and args.breakdown:
    for i in range(n_env):
        ...
        for j in range(1, n_brk):
            ...
            oth['func_method'] = partial(Eval, oth['spline'])
            oth['func'] = ROOT.TF1('splinefn' + str(NAMECOUNTER), oth['func_method'], oth[
                                     'graph'].GetX()[0], oth['graph'].GetX()[oth['graph'].GetN() - 1], 1)
            oth['func']._method = oth['func_method']
            oth['func'].SetNpx(NPX)
    ...
    for other in new_others:
        ...
        other['func_method'] = partial(Eval, other['spline'])
        other['func'] = ROOT.TF1('splinefn' + str(NAMECOUNTER), other['func_method'], other[
                                 'graph'].GetX()[0], other['graph'].GetX()[other['graph'].GetN() - 1], 1)
        other['func']._method = other['func_method']
...
if args.hide_envelope:
    main_scan['graph'].SetMarkerSize(0)
main_scan['graph'].SetMarkerColor(args.main_color)
...
for other in other_scans:
    ...
    if args.drop_func:
        other['graph'].SetMarkerSize(0.7)
        other['graph'].SetMarkerStyle(24)
    other['graph'].Draw('PSAME')
...
if args.decorate_signif or args.paper:
    ...
    plot.DrawCMSLogo(pads[0], 'CMS',
                    'Supplementary', 11, 0.045, 0.035, 1.2, '', 1.1 if args.pub else 0.8)
else:
    plot.DrawCMSLogo(pads[0], 'CMS',
...
if args.drop_func:
    legend.AddEntry(main_scan['graph'], args.main_label, 'P')
    for i, other in enumerate(other_scans):
        legend.AddEntry(other['graph'], other_scans_opts[i][1], 'P')
else:
